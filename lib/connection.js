...
function Connection(host, port, pingUri) {
  ...
  //define vars for data packet parsing
  this.openBrackets = 0;
  this.buffer = '';
  this.prevChar = '';
  this.isQuoted = false;
...
Connection.prototype.send = function(request, callback) {
  //register the callbacks
  this.callbacks[request.txnId()] = callback;
  ...
  for (var c in data) {
    ...
    if (c === '}') {
      ...
      if (this.openBrackets === 0) {
        //complete packet.
        json = this.buffer;
        //TODO: parse json into response and send to appropriate callback
        var response = new Response();
        response.fromJSON(JSON.parse(json));

        this.buffer = '';
      }
    }
  }
  //TODO: sanity check buffer size.  if greater then some threshold, disconnect throw error
}
